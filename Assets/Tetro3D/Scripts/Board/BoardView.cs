...
public class BoardView : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _transparentCubePrefab;
    ...
    public void GenerateGrid(int width, int height, int depth)
    {
        ...
        GenerateTransparentFloor(width, depth, platformYPos);
    ...
    private void GenerateTransparentFloor(int width, int depth, int yPos)
    {
        Vector3 size = new Vector3(width, 1f, depth);
        Vector3 centerPos = new Vector3((width - 1) / 2f, yPos, (depth - 1) / 2f);

        GameObject cube = Instantiate(_transparentCubePrefab, centerPos, Quaternion.identity, _platform.transform);
